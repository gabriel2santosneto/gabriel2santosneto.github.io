...
function Notepad(file_path) {
  ...
  var $win = make_iframe_window({
    src:
      "programs/notepad/index.html" +
      (file_path ? "?path=" + encodeURIComponent(file_path) : ""),
...
function Explorer(address) {
  ...
  // Calculate available height accounting for taskbar
  const $taskbar = $(".taskbar");
  const taskbar_height = $taskbar.length ? $taskbar.outerHeight() + 1 : 0;
  const available_height = innerHeight - taskbar_height;
  ...
  var $win = make_iframe_window({
    ...
    innerHeight: Math.min(547, available_height * 0.7),
    constrainRect: function (rect, x_axis, y_axis) {
      // Constrain window to not overlap taskbar
      const $taskbar = $(".taskbar");
      const taskbar_height = $taskbar.length ? $taskbar.outerHeight() + 1 : 0;
      const max_available_height = innerHeight - taskbar_height;

      // Constrain height
      if (rect.height > max_available_height) {
        rect.height = max_available_height;
      }
      // Constrain bottom position to not go below taskbar
      if (rect.y + rect.height > max_available_height) {
        rect.y = Math.max(0, max_available_height - rect.height);
      }
      return rect;
    },
  });
...
function openWelcomeWindow() {
  if (typeof showCustomMessageBox !== "function") {
    const $win = make_iframe_window({
      src: "programs/welcome/index.html",
      icons: iconsAtTwoSizes("favorites"),
      title: "Welcome to my Portfolio",
      innerWidth: Math.min(860, innerWidth * 0.85),
      innerHeight: Math.min(560, innerHeight * 0.8),
    });
    const task = new Task($win);
    const recenterFallback = () => {
    ...
    if ($win.$iframe) {
      $win.$iframe.on("load", recenterFallback);
    }
    recenterFallback();
    return task;
  }
  ...
  const welcomeParagraphs = [
    "Welcome to my personal website, an interactive portfolio that shows how I bring creativity, robotics, and innovation into learning.",
    "I teach and design programs for schools, parents, and communities — from after-school clubs to full STEM curriculums.",
    "Explore the system, experiment with the windows, and discover how education can feel alive again.",
  ];

  const children = welcomeParagraphs.map((text) => $("<p>").text(text));

  const welcomeBox = showCustomMessageBox({
    title: "Welcome to my Portfolio",
    heading: "Welcome",
    headingIcon: "images/icons/windows-32x32-8bpp.png",
    headingIconAlt: "Windows logo",
    children,
    buttons: [{ label: "START", value: "close", default: true }],
    width: 560,
    wrapperClass: "custom-msgbox--welcome",
    bodyClass: "custom-msgbox__body--welcome",
  });

  return welcomeBox;
}

function openPortfolioExplorer(section) {
  ...
  withFilesystem(function () {
    ...
    fs.readFile(file_path, "utf8", function (err, content) {
      ...
      if (url) {
        ...
        if (normalizedUrl.toLowerCase().startsWith("program:")) {
          const programName = normalizedUrl.slice("program:".length);
          if (launchProgramShortcut(programName)) {
            return;
          }
        }
      }
...
function launchProgramShortcut(programName) {
	if (!programName) {
		return false;
	}
...
// Note: `C:\Windows\Desktop` doesn't contain My Computer, My Documents, Recycle Bin, or Internet Explorer,
...
add_icon_not_via_filesystem({
  ...
  open: function () {
    ...
    const viewportWidth =
      window.innerWidth || document.documentElement.clientWidth || 550;
    const viewportHeight =
      window.innerHeight || document.documentElement.clientHeight || 450;
    ...
    const innerHeight = Math.min(
      targetHeight,
      Math.max(320, viewportHeight - 48)
    );
    var $win = make_iframe_window({
...
function Junkbot() {
  ...
  // Open Junkbot game in Explorer (iframe) for full immersion
  var $win = make_iframe_window({
    src: "https://1j01.github.io/janitorial-android/#junkbot",
    icons: iconsAtTwoSizes("tools-folder"), // Using tools-folder icon for LEGO/robotics theme
    title: "Junkbot.exe",
    innerWidth: Math.min(1024, innerWidth * 0.9),
    innerHeight: Math.min(768, innerHeight * 0.8),
  });
  return new Task($win);
}
Junkbot.acceptsFilePaths = false;
